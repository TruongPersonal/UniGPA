...
Future<void> showAddSubjectSheet(
  ...
  return AppBottomSheet.show(
    ...
    builder: (ctx) => MultiProvider(
      ...
      child: _AddSubjectSheet(editIndex: editIndex, editSubject: editSubject),
    ),
  );
}
...
class _AddSubjectSheetState extends State<_AddSubjectSheet> {
  ...
  void initState() {
    ...
    _isViewingMode =
        ...
        s?.finalPoint10 == null &&
        s?.processPoint != null &&
        s?.processWeight != null;

    _nameCtrl = TextEditingController(text: s?.name ?? '')
      ..addListener(_onInputChanged);
    _creditsCtrl = TextEditingController(text: s != null ? '${s.credits}' : '')
      ..addListener(_onInputChanged);

    _processWeightCtrl = TextEditingController(
      text: s?.processWeight != null
          ? (s!.processWeight! * 100).toStringAsFixed(0)
          : '',
    )..addListener(_onInputChanged);
